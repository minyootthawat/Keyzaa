  ...
  return (
    <AdminRouteGuard requiredPermission="admin:products:read">
      <div className="space-y-6 md:space-y-7">
                          ...
                          <div className="flex justify-end gap-1.5">
                            ...
                            {isSuperAdmin && (
                              <button
                                ...
                                className="shrink-0 rounded-xl border border-error/40 bg-error/10 px-3 py-1.5 text-xs font-semibold text-error transition-opacity hover:bg-error/20"
                              >
                                {lang === "th" ? "ลบ" : "Delete"}
                              </button>
                            )}
              ...
              <div className="flex items-center justify-between">
                <p className="text-sm text-text-muted">
                  {lang === "th"
                    ...
                    : `Showing ${(page - 1) * ITEMS_PER_PAGE + 1}–${Math.min(page * ITEMS_PER_PAGE, total)} of ${total} items`}
                </p>
                <div className="flex gap-2">
                  <button
                    onClick={() => {
                      ...
                      setPage(newPage);
                      fetchProducts(newPage, filter, search);
                    }}
                    disabled={page === 1}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
            <div className="surface-card w-full max-w-md rounded-2xl p-6">
              ...
              <div className="space-y-4">
                <div>
                  <label className="mb-1.5 block text-sm font-semibold text-text-subtle">
                    {lang === "th" ? "ราคา (฿)" : "Price (฿)"}
                  </label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="mb-1.5 block text-sm font-semibold text-text-subtle">
                    {lang === "th" ? "จำนวนสต็อก" : "Stock Quantity"}
                  </label>
                  <input
                    type="number"
                    min="0"
              ...
              <div className="mt-6 flex justify-end gap-3">
                <button
                  ...
                  disabled={savingEdit}
                  className="rounded-xl border border-border-subtle bg-bg-surface px-4 py-2 text-sm font-semibold text-text-subtle transition-colors hover:border-border-main hover:text-text-main"
                >
        ...
        {deleteProduct && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
            <div className="surface-card w-full max-w-sm rounded-2xl p-6">
              <h2 className="type-h2 mb-2 text-error">
                {lang === "th" ? "ลบสินค้า?" : "Delete Product?"}
              </h2>
              <p className="mb-1 text-sm text-text-main font-semibold">
                {deleteProduct.name}
              </p>

              {orderCountWarning !== null ? (
                <div className="mt-3 rounded-xl border border-warning/40 bg-warning/10 px-4 py-3 text-sm text-warning">
                  ⚠️{" "}
                  {lang === "th"
                    ? `สินค้านี้มี ${orderCountWarning} ออเดอร์ที่ยังดำเนินอยู่ ไม่สามารถลบได้`
                    : `This product has ${orderCountWarning} existing orders. Cannot delete.`}
                </div>
              ) : (
                <p className="mt-3 text-sm text-text-muted">
                  {lang === "th"
                    ? "การลบสินค้าจะไม่สามารถกู้คืนได้ ยืนยันที่จะลบหรือไม่?"
